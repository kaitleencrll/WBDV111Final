...
// Function to handle Organization Approval
function approveOrg(orgName, elementId) {
    const confirmed = confirm(`Are you sure you want to approve ${orgName} as a verified partner?`);
    
    if (confirmed) {
        // 1. Alert the user
        alert(`${orgName} has been moved to Verified Partners.`);

        // 2. Hide the card from the pending list
        const card = document.getElementById(elementId);
        if (card) {
            card.style.display = 'none';
        }

        // NOTE: In a real app, you would also use fetch() to update your 
        // database here or update your localStorage list.
    }
}

// Function to simulate viewing documents
function viewDocuments(orgName) {
    alert(`Opening secure document viewer for: ${orgName}\n\n1. business_permit.pdf\n2. rescue_cert.pdf\n3. vet_affiliation.jpg`);
    
    // You could also use window.open('path/to/folder') or trigger a modal here.
}
// adminscript.js
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    counters.forEach(counter => {
        ...
        const count = target > 1000 ? target - 100 : 0;
        counter.innerText = count;
        
        const updateCount = () => {
            const current = +counter.innerText;
            if (current < target) {
                counter.innerText = current + Math.ceil(target / 50);
...
function renderAdminContent() {
    // Render Admin Registry
    const registry = document.getElementById('adminRegistryList');
    const admins = [
        { name: "Valenzuela HQ", type: "Main Branch", status: "Active" },
        { name: "Paws & Claws", type: "Partner Clinic", status: "Active" }
    ];
    registry.innerHTML = admins.map(a => `
        <div class="partner-item">
            <div class="p-img" style="background: #1e293b; color: white; display: flex; align-items: center; justify-content: center; font-weight: bold;">
                ${a.name.charAt(0)}
            </div>
            <div class="p-info">
                <span class="v-badge">${a.status}</span>
                <h4>${a.name}</h4>
                <small>${a.type}</small>
            </div>
        </div>
